		...
		if type_def.has("struct"):
			...
			if type_def.has("table_names"):
				if not type_def.has("primary_key_name"):
					SpacetimePlugin.print_log("Warning: struct %s has no primary key" % type_def.get("name", ""))
					continue
	...
	for _type_def in schema.types:
		...
		if _type_def.has("table_name"):
			if not _type_def.has("primary_key_name"):
				SpacetimePlugin.print_log("Warning: struct %s has no primary key" % _type_def.get("name", ""))
				continue
